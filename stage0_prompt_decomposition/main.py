...
def main(
    ...
    zero_stage=3,
    output_dir="./hcx_opt_7b_slr_aqqa",
    data_output_path="./data_files/",
    offload=True,
    dtype="bf16",
    compute_fp32_loss=False,
    enable_tensorboard=False,
    tensorboard_path="step1_tensorboard",
    add_eot_token=False,
    print_loss=True,
    ...
    tokenizer = load_hf_tokenizer(
        ...
        add_special_tokens=["<|user|>", "<|assistant|>", "<|order_0|>", "<|order_1|>"]
    ...
    # set trainable param to only 1/4 of the model, to save memory
    random.seed(0)
    for n, p in model.named_parameters():
        if (
            len(p.shape) >= 1
            and random.random() < replace_chance
            and not any(s in n for s in ["lm_head", "token"])
        ):
    ...
    optimizer_grouped_parameters = get_optimizer_grouped_parameters(
        model, weight_decay, embed_lr=learning_rate * 100
    )
            ...
            if global_step % save_freq == 10:
                ...
                if output_dir is not None:
                    ...
                    if global_rank == 0:
                        ...
                        saving_output_dir = os.path.join(
                            output_dir, f"step_{global_step}_final"
                        )
                        os.makedirs(saving_output_dir, exist_ok=True)
                        output_model_file = os.path.join(
                            saving_output_dir, WEIGHTS_NAME
                        )
                        output_config_file = os.path.join(
                            saving_output_dir, CONFIG_NAME
                        )
                    ...
                    if zero_stage == 3:
                        ...
                        save_zero_three_model(
                            model, global_rank, saving_output_dir, zero_stage=zero_stage
                        )
