...
class PreprocessedDataset(Dataset):
    def __init__(
        ...
        self.caption_2 = self.data["style"]
        ...
        for key, value in substitute_caption_map.items():
            self.caption = self.caption.str.replace(key, value)

        self.image_path = self.data["img_path"]
    ...
    def _process(
        ...
        caption_2 = self.caption_2[idx]
          # with random chance, replace captions with ''.
        ...
        print(f"Data {idx}, caption: {caption}, caption_2: {caption_2}")
...
def register_hook(layer):
    def hook_fn(module, grad_input, grad_output):
        # grad_output is a tuple, where we're interested in the first element
        grad = grad_output[0]
        if grad is not None:
            # Calculate quantiles
            grad = grad.flatten().abs().float()
            # random sample 1000 elements
            grad = grad[torch.randint(0, len(grad), (1000,))]
            q25 = torch.quantile(grad, 0.25).item()
            median = torch.median(grad).item()
            q75 = torch.quantile(grad, 0.75).item()
            ...
            if grad_max > 1e-3:
                print(
                    f"Max Gradient: {grad_max}, 75th Quantile: {q75}, Median: {median}, 25th Quantile: {q25}, Min Gradient: {grad_min}"
                )

    return layer.register_full_backward_hook(hook_fn)


def register_gradient_logger(model):
...
class TokenEmbeddingsHandler:
    ...
    def initialize_new_tokens(self, inserting_toks: List[str]):
        ...
        for tokenizer, text_encoder in zip(self.tokenizers, self.text_encoders):
            ...
            print(f"Inserting tokens: {self.inserting_toks}")
            for tok in self.inserting_toks:
                assert (
                    tok not in tokenizer.all_special_tokens
                ), f"New tokens should be special., {tok} in {tokenizer.all_special_tokens}"
    ...
    def save_embeddings(self, file_path: str):
        ...
        save_file(
            tensors,
            file_path,
            metadata={"inserting_toks": ",".join(self.inserting_toks)},
        )

        if self.previous_embeddings is not None:
            print(
                f"Embedding change: {torch.norm(self.previous_embeddings - new_token_embeddings)}"
            )
            print(
                f"Embedding change in cosine similarity: {torch.nn.functional.cosine_similarity(self.previous_embeddings, new_token_embeddings, dim=-1).mean()}"
            )

        self.previous_embeddings = new_token_embeddings
