...
class BasicTransformerBlock(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.attn3 = Attention(hidden_size, 768)
        self.norm4 = nn.LayerNorm(hidden_size, eps=1e-05, elementwise_affine=True)

        self.ff = FeedForward(hidden_size, hidden_size)

    def _copy_weight_attn3_from_attn2(self):
        # initialize attn3 q k v o as same as attn2, but with different dim
        self.attn3.to_q.weight.data = self.attn2.to_q.weight.data
        self.attn3.to_k.weight.data = self.attn2.to_k.weight.data[:, :768]
        self.attn3.to_v.weight.data = self.attn2.to_v.weight.data[:, :768]
        self.attn3.to_out[0].weight.data = self.attn2.to_out[0].weight.data
        self.attn3.to_out[0].bias.data = self.attn2.to_out[0].bias.data
